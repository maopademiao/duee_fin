...
log_dir=./log
submit_data_path=./submit/new2/test_duee_fin2.json
pred_data=${data_dir}/sentence/test1.json  # 换其他数据，需要修改它
...
run_sequence_labeling_model(){
    model=${1}
    is_train=${2}
    pred_save_path=${ckpt_dir}/${model}/test1_pred.json
    nohup sh run_sequence_labeling.sh ${data_dir}/${model} ${conf_dir}/${model}_tag.dict ${ckpt_dir}/${model} ${pred_data} ${learning_rate} ${is_train} ${max_seq_len} ${batch_size} ${epoch} ${pred_save_path} ${log_dir}/${process_name}.log > ${log_dir}/${model}_nohup.out 2>&1 &
}
...
run_classifier_model(){
    ...
    sh run_classifier.sh ${data_dir}/${model} ${conf_dir}/${model}_tag.dict ${ckpt_dir}/${model} ${pred_data} ${learning_rate} ${is_train} ${max_seq_len} ${batch_size} ${epoch} ${pred_save_path} ${process_name}
}
...
if [ ${process_name} == data_prepare ]; then
    ...
    python duee_fin_post2.py --trigger_file ${ckpt_dir}/trigger/test_pred2.json --role_file ${ckpt_dir}/role/test_pred2.json --enum_file ${ckpt_dir}/enum/test_pred2.json --schema_file ${conf_dir}/event_schema.json --save_path ${submit_data_path}
