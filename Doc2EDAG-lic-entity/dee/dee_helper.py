...
from .event_type import event_type2event_class, BaseEvent, event_type_fields_list, field2biolabel
...
class DEEExample(object):
        ...
        if only_inference:
            ...
            for mspan in self.ann_valid_mspans:
                for drange in self.ann_mspan2dranges[mspan]:
                    ...
                    '''if guess_field in ["质押方","减持方", "债权人", "增持方","投资方"]:
                        if len(guess_field)>=4:
                            guess_field = "公司名"
                        else:
                            guess_field = "人名"
                    else:
                        for key,item in field2biolabel.items():
                            if guess_field in item:
                                guess_field = key
                                break'''
    ...
    def get_entity_label_list():
        ...
        for fields in [['人名', '公司名', '数字','金钱',  '时间', '职位', '中标标的', '收购标的', '融资轮次', '变动类型','质押物']]:
            for field in fields:
                if field not in visit_set:
                    visit_set.add(field)
                    entity_label_list.extend(['B-' + field, 'I-' + field])
...
class DEEFeatureConverter(object):
    ...
    def convert_example_to_feature(self, ex_idx, dee_example, log_flag=False):
        ...
        if len(span_token_ids_list) == 0 and not dee_example.only_inference:
            logger.warning('Neglect example {}'.format(annguid))
            return None
